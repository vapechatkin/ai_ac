...
# ── Invariant memory (invariants.json) ────────────────────────────────────────
...
INVARIANTS_FILE = DIR / "invariants.json"


class InvariantMemory:
    """
    Invariants are hard constraints the assistant must never violate.
    Stored in invariants.json as a list of records.
      ...
      "invariants": [
        {
          "id":         "a1b2c3d4",
          "category":   "stack",
          "rule":       "Использовать только Kotlin, без Java",
    ...
    def _load(self) -> dict:
        if INVARIANTS_FILE.exists():
            try:
                return json.loads(INVARIANTS_FILE.read_text("utf-8"))
            except json.JSONDecodeError:
                pass
        return {"invariants": []}
    ...
    def get_all(self) -> list[dict]:
        return self._load()["invariants"]

    def add(self, category: str, rule: str) -> str:
        if category not in INVARIANT_CATEGORIES:
            cats = ", ".join(INVARIANT_CATEGORIES)
            return f"Неизвестная категория '{category}'. Доступны: {cats}"
        if not rule.strip():
    ...
    def remove(self, inv_id: str) -> str:
        data = self._load()
        before = len(data["invariants"])
        data["invariants"] = [i for i in data["invariants"] if i["id"] != inv_id]
        if len(data["invariants"]) == before:
            return f"Инвариант '{inv_id}' не найден"
        self._save(data)
        return ""

    def to_prompt_block(self) -> str:
        """Return formatted block for injection into system prompt."""
        invs = self.get_all()
        if not invs:
            return ""
        by_cat: dict[str, list[str]] = {}
        for inv in invs:
            by_cat.setdefault(inv["category"], []).append(inv["rule"])
        lines = ["## ИНВАРИАНТЫ — ЖЁСТКИЕ ОГРАНИЧЕНИЯ (нарушать запрещено)", ""]
        for cat, rules in by_cat.items():
            label = INVARIANT_CATEGORIES.get(cat, cat)
            lines.append(f"### {label}")
            for r in rules:
                lines.append(f"- {r}")
            lines.append("")
        lines.append(
            "ПРАВИЛО: Если запрос пользователя противоречит любому инварианту — "
            "ОТКАЖИ, явно назови нарушенный инвариант и объясни почему это ограничение важно. "
            "Не предлагай обходных путей, нарушающих инвариант."
        )
        return "\n".join(lines)
    ...
    def summary(self) -> str:
        ...
        by_cat: dict[str, int] = {}
        for inv in invs:
            by_cat[inv["category"]] = by_cat.get(inv["category"], 0) + 1
        parts = [f"{INVARIANT_CATEGORIES.get(c, c)}: {n}" for c, n in by_cat.items()]
        return f"{len(invs)} инвариантов — " + ", ".join(parts)
